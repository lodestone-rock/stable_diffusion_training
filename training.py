...
tokenizer = CLIPTokenizer.from_pretrained(config_dict["model_path"], subfolder="tokenizer")
dataloader = DataLoader(
    tokenizer_obj=tokenizer,
    config=config_dict_path,  # Replace Any with the actual type of creds_data
    ramdisk_path=config_dict["ramdisk_path"], # temp storage path preferably ramdisk because it's fast
    training_batch_size=config_dict["batch_size"], # trainig batch
    repeat_batch=config_dict["repeat_batch"], # number of same resolution repeated during data shuffling (prevent jax switching compiled function back and forth)
    maximum_resolution_areas=[x**2 for x in config_dict["image_area_root"]], # bucketing area target
    bucket_lower_bound_resolutions=config_dict["minimum_axis_length"], # bucketing minimum resolution axis list
    numb_of_worker_thread=config_dict["numb_of_dataloader_worker_thread"], # dont set this too high it will consume a lot of RAM
    queue_get_timeout=config_dict["queue_get_timeout"], # timeout to indicate end of batch 
    # init value and will get incremented inside loop
    chunk_number=config_dict["chunk_number"],  # This should be incremented for each successful data loading
    seed=config_dict["master_seed"],  # This should be incremented when all batches are processed
)
...
unet_state, text_encoder_state, frozen_vae, frozen_schedulers = on_device_model_training_state(training_config)
...
for _ in range(config_dict["chunk_limit"]):
    ...
    chunk = config_dict["chunk_number"] # this state is stored in dict and will get updated
    ...
    train_step_funcs = dp_compile_all_unique_resolution(unet_state, text_encoder_state, frozen_vae, frozen_schedulers, training_config)
    ...
    # while true ?
    for count in range(int(dataloader._bulk_batch_count + dataloader._first_batch_count)):
        ...
        current_batch["input_ids"] = current_batch["input_ids"].reshape(-1, config_dict["text_encoder_context_window"])
        current_batch["attention_mask"] = current_batch["attention_mask"].reshape(-1, config_dict["text_encoder_context_window"])
        # just progress bar update
        train_step_progress_bar.update(1)

        unet_state, text_encoder_state, train_metric, train_rngs = train_step_funcs[current_batch["pixel_values"].shape](
                # donated args
...
# flush storage clean
for flushed_batch in range(config_dict["chunk_limit"]+config_dict["numb_of_prefetched_batch"]):
