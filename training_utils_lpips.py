...
def load_models(training_config: TrainingConfig) -> dict:
    ...
    with jax.default_device(jax.devices("cpu")[0]):
        ...
        # LPIPS model for calculating similarity loss
    
        lpips_model = LPIPS()
        def _init_lpips(rng, image_size):
            x = jnp.zeros(shape=(1, image_size, image_size, 3))
            return lpips_model.init(rng, x, x)

        # lpips must use rng for some reason to init the model soo ¯\_(ツ)_/¯    
        lpips_params = _init_lpips(jax.random.PRNGKey(0), image_size=256)
    ...
    return {
        ...
        "lpips": {
            "lpips_params": jax.tree_map(lambda x: x.astype(jnp.bfloat16), lpips_params),
            "lpips_model": lpips_model,
        },
    }
...
def create_frozen_states(models: dict):
    ...
    lpips_state = FrozenModel(
        call=models["lpips"]["lpips_model"],
        params=models["lpips"]["lpips_params"],
    )
    ...
    return {"vae_state": vae_state, "schedulers_state": schedulers_state, "lpips_state":lpips_state}
...
def on_device_model_training_state(training_config: TrainingConfig):
    ...
    frozen_lpips_adapter = jax.device_put(
        jnp.array([
...
def train_step(
    ...
    frozen_lpips_state: Any,
    frozen_lpips_adapter: jax.Array,
    # static args
