...
def load_models(training_config: TrainingConfig) -> dict:
    ...
    return {
        ...
        "text_encoder_2": {
            "text_encoder_2_params": text_encoder_2_params,
...
def create_lion_optimizer_states(
    ...
    text_encoder_2_learning_rate: float = 1e-6,
    excluded_layer_pattern_from_weight_decay: list = [],
    excluded_layer_from_quantization: list = [],
    ...
    with jax.default_device(jax.devices("cpu")[0]):
        ...
        if train_text_encoder:
            ...
            text_encoder_2_constant_scheduler = optax.constant_schedule(
                text_encoder_2_learning_rate / adam_to_lion_scale_factor
            )
            if quantize_text_encoder_state:
                ...
                text_encoder_2_quant_mask = create_mask(
                    models["text_encoder_2"]["text_encoder_2_params"],
                    excluded_layer_from_quantization,
                )
                text_encoder_2_lion = lion_8bit(
                    learning_rate=text_encoder_2_constant_scheduler,
                    b1=0.9,
                    b2=0.99,
                    weight_decay=1e-2 * adam_to_lion_scale_factor,
                    mask=text_encoder_weight_decay_mask,
                    block_size=lion_8bit_block_size,
                    excluded_layer_mask=text_encoder_2_quant_mask,
                )
            else:
                ...
                text_encoder_2_lion = optax.lion(
                    learning_rate=text_encoder_2_constant_scheduler,
                    b1=0.9,
                    b2=0.99,
                    weight_decay=1e-2 * adam_to_lion_scale_factor,
            ...
            text_encoder_2_optimizer = optax.chain(
                optax.clip_by_global_norm(1),  # prevent explosion
                text_encoder_2_lion,
            )
            ...
            text_encoder_2_state = train_state.TrainState.create(
                # transformer implementation does not have apply method apparently
                apply_fn=models["text_encoder_2"]["text_encoder_2_model"].__call__,
                params=models["text_encoder_2"]["text_encoder_2_params"],
                tx=text_encoder_2_optimizer,
            )

    return {"unet_state": unet_state, "text_encoder_state": text_encoder_state, "text_encoder_2_state":text_encoder_2_state}
...
def on_device_model_training_state(training_config: TrainingConfig):
    ...
    text_encoder_2_state = jax.tree_map(
        lambda leaf: jax.device_put(leaf, device=NamedSharding(mesh, PartitionSpec())),
        trained_model_states["text_encoder_2_state"],
    )
    ...
    if training_config.accumulate_text_encoder_ema:
        ...
        text_encoder_2_ema_params = jax.tree_map(
            lambda leaf: jax.device_put(
                leaf, device=NamedSharding(mesh, PartitionSpec())
            ),
            models["text_encoder_2"]["text_encoder_2_params"],
        ...
        text_encoder_2_ema_params = None
    ...
    model_object_dict = {
        ...
        "text_encoder_2": models["text_encoder_2"]["text_encoder_2_model"],
...
def train_step(
    ...
    text_encoder_2_state: Any,  # define sharding rule!
    unet_ema_params: dict,
    text_encoder_ema_params: dict,
    text_encoder_2_ema_params: dict,
    # variable args
    ...
    def concatenate_text_encoder_latent(
            encoder_hidden_states:jnp.array, 
            batch_size:int,
            strip_bos_eos_token:bool
    ):
        """A hacky way to insert more context length by simply concatenating embedding along sequence dimension"""
        # reshape encoder_hidden_states to shape (batch, token_append, token, hidden_states)
        encoder_hidden_states = jnp.reshape(
            encoder_hidden_states,
            (batch_size, -1, 77, encoder_hidden_states.shape[-1]),
        )
    ...
    def get_res_cond_to_time_proj(original_size, crops_coords_top_left, target_size, bs, dtype):
        """
        this is sdxl hack to make sure they can use and augment their dataset due to most of the data distribution
        is a low resolution images. by providing the contidional info to the embedding layer they can "cheat" and train
        a high res model without having to upscale all of their dataset.
        """
    ...
    def compute_loss(
        unet_params, text_encoder_params, text_encoder_2_params, vae_params, noise_scheduler_state, batch
    ):
        ...
        # TODO: add a way to disable this layer (tbh i should just make a function to prevent any layer to be trained based on the layer keys)
        # NOTE: since SDXL uses projection layer, i think it's wise to include or atleast average the BOS token during prompt extension
        # because BOS token act as attention sink here.
        # text encoder 1
        print(batch["input_ids"].shape)
        encoder_hidden_states = text_encoder_state.apply_fn(
            params=text_encoder_params,
            input_ids=batch["input_ids"], # TODO: input ids should be different here !
            train=True,
        ...
        # text encoder 2
        print(batch["input_ids"].shape)
        encoder_2_hidden_states = text_encoder_2_state.apply_fn(
            params=text_encoder_2_params,
            input_ids=batch["input_ids_2"], # TODO: input ids should be different here !
            train=True,
            output_hidden_states=True
        )
    ...
    grad_fn = jax.value_and_grad(
        fun=compute_loss, argnums=[0, 1, 2]  # differentiate first and second input 
        # TODO add a way to tell jax which args is differentiable
    )
    ...
    if ema_rate and text_encoder_2_ema_params:
        print(ema_rate)
        new_text_encoder_2_ema_params = compute_model_ema(
            text_encoder_2_ema_params, new_text_encoder_2_state.params
        )
    else:
        new_text_encoder_2_ema_params = None
    ...
    return (
        ...
        new_text_encoder_2_state,
        new_unet_ema_params,
        new_text_encoder_ema_params,
        new_text_encoder_2_ema_params,
        metrics,
        new_train_rng,
    )
    ...
    def _create_lowered_hlo(
        ...
        p_train_step = jax.jit(
            ...
            donate_argnums=(
                ...
                2,  # "text_encoder_2_state"
                3,  # if training_config.accumulate_unet_ema else None,
                4,  # if training_config.accumulate_text_encoder_ema else None,
                5,  # if training_config.accumulate_text_encoder_2_ema else None,
            ),
            in_shardings=(
                # unet_state
                jax.tree_map(
        ...
        with TimingContextManager(f"lowering {bucket_resolution}"):
            lowered_hlo = p_train_step.lower(
                ...
                text_encoder_2_state,  # text_encoder_state
                unet_ema_params,
                text_encoder_ema_params,
                text_encoder_2_ema_params,
                # variable args
...
def save_model(
    ...
    tokenizer_2_object: Any,
    unet_params: dict,
    text_encoder_params: dict,
    text_encoder_2_params: dict,
    vae_params: dict,
    output_dir: str,
):
    ...
    pipeline = FlaxStableDiffusionXLPipeline(
        ...
        tokenizer_2=tokenizer_2_object,
        text_encoder=model_object_dict["text_encoder"],
        text_encoder_2=model_object_dict["text_encoder_2"],
