...
import numpy as np
from training_utils_sdxl_distilled import (
    TrainingConfig,
    on_device_model_training_state,
    dp_compile_all_unique_resolution,
    save_model,
)
from streamer.dataloader import SDXLDataLoader
...
def main():
    ...
    tokenizer_2 = CLIPTokenizer.from_pretrained(
        config_dict["model_path"], subfolder="tokenizer_2"
    )
    dataloader = SDXLDataLoader(
        tokenizer_obj=tokenizer,
        tokenizer_2_obj=tokenizer_2,
    ...
    for _ in range(config_dict["chunk_limit"]):
        ...
        try:
            ...
            save_model(
                ...
                tokenizer_2_object=tokenizer_2,
                unet_params=unet_state.params,
                text_encoder_params=text_encoder_state.params,
                text_encoder_2_params=text_encoder_2_state.params,
                vae_params=frozen_vae.params,
                output_dir=config_dict["test_save_path"],
            )
            if config_dict["ema_rate"]:
                save_model(
                    model_object_dict=model_object_dict,
                    tokenizer_object=tokenizer,
                    tokenizer_2_object=tokenizer_2,
                    unet_params=unet_ema_params
                    if config_dict["accumulate_unet_ema"]
                    else unet_state.params,
                    text_encoder_params=text_encoder_ema_params
                    if config_dict["accumulate_text_encoder_ema"]
                    else text_encoder_state.params,
                    text_encoder_2_params=text_encoder_2_ema_params
                    if config_dict["accumulate_text_encoder_2_ema"]
                    else text_encoder_2_state.params,
                    vae_params=frozen_vae.params,
                    output_dir=f"{config_dict['test_save_path']}-EMA",
                )
        ...
        delete_file_or_folder(f"{config_dict['test_save_path']}-EMA")
        ...
        for count in range(
            ...
            current_batch["input_ids_2"] = current_batch["input_ids_2"].reshape(
                -1, config_dict["text_encoder_context_window"]
            )
            ...
            (
                ...
                train_metric,
                train_rngs,
            ) = train_step_funcs[current_batch["pixel_values"].shape](
                # donated args
        ...
        if config_dict["ema_rate"]:
            latest_ema_model_path = (
                f'{model_path_without_chunk_number}-EMA@{config_dict["chunk_steps"]}'
            )
            save_model(
                model_object_dict=model_object_dict,
                tokenizer_object=tokenizer,
                ...
                vae_params=frozen_vae.params,
                output_dir=latest_ema_model_path,
            )
            # only save n latest chunk so it not cluttering the storage
            delete_file_or_folder(
                f'{model_path_without_chunk_number}-EMA@{config_dict["chunk_steps"]-config_dict["keep_trained_model_buffer"]}'
            )
